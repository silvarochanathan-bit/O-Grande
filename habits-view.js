...
window.HabitView = {
    ...
    // =========================================================================
    // CALENDÁRIOS DE HÁBITOS (GRID DE PONTOS, 3 ESTILOS)
    // =========================================================================

    _monthNames: ['Jan','Fev','Mar','Abr','Mai','Jun','Jul','Ago','Set','Out','Nov','Dez'],
    _weekdayLetters: ['D','S','T','Q','Q','S','S'],

    renderCalendarsGrid: function(habits, monthOffsets, style) {
        const container = document.getElementById('calendars-grid-container');
        if (!container) return;
        container.innerHTML = '';

        const visibleHabits = habits.filter(h => !h.isDependent);

        if (visibleHabits.length === 0) {
            container.innerHTML = '<div style="text-align:center; opacity:0.5; padding:30px; font-size:0.9rem;">Nenhum hábito cadastrado ainda.</div>';
            return;
        }

        visibleHabits.forEach(habit => {
            container.appendChild(this._renderSingleHabitCalendar(habit, monthOffsets[habit.id] || 0, style));
        });
    },

    _renderSingleHabitCalendar: function(habit, monthOffset, style) {
        const now = new Date();
        const base = new Date(now.getFullYear(), now.getMonth() + monthOffset, 1);
        const year = base.getFullYear();
        const month = base.getMonth();
        const firstWeekday = base.getDay(); // 0=Dom
        const cells = window.HabitModel.getCalendarDataForMonth(habit, year, month);

        const card = document.createElement('div');
        card.className = 'habit-cal-card';
        card.setAttribute('data-style', style);

        const freqLabel = this._getFrequencyLabel(habit);

        card.innerHTML = `
            <div class="habit-cal-header">
                <span class="habit-cal-name">${habit.name}</span>
                <div class="habit-cal-nav">
                    <button class="habit-cal-nav-btn" data-dir="-1">‹</button>
                    <span class="habit-cal-month">${this._monthNames[month]} ${year}</span>
                    <button class="habit-cal-nav-btn" data-dir="1">›</button>
                </div>
            </div>
            <div class="habit-cal-freq">${freqLabel}</div>
            <div class="habit-cal-weekdays">${this._weekdayLetters.map(l => `<span>${l}</span>`).join('')}</div>
            <div class="habit-cal-grid"></div>
        `;

        card.querySelectorAll('.habit-cal-nav-btn').forEach(btn => {
            btn.onclick = () => window.HabitManager.changeCalendarMonth(habit.id, parseInt(btn.dataset.dir));
        });

        const gridEl = card.querySelector('.habit-cal-grid');

        // Espaços vazios antes do dia 1, para alinhar com o dia da semana correto
        for (let i = 0; i < firstWeekday; i++) {
            const empty = document.createElement('div');
            empty.className = 'habit-cal-dot na';
            gridEl.appendChild(empty);
        }

        cells.forEach(cell => {
            const dot = document.createElement('div');
            const statusClass = cell.status === 'not-scheduled' ? 'na' : cell.status;
            dot.className = `habit-cal-dot ${statusClass}`;

            if (cell.status === 'done' || cell.status === 'missed') {
                dot.innerHTML = `<span class="cal-day-num-main">${cell.day}</span><span class="cal-day-num">${cell.day}</span>`;
                if (style === 'thumbs') {
                    const shape = document.createElement('div');
                    shape.className = 'cal-thumb-shape';
                    dot.appendChild(shape);
                }
                // Só dias passados podem ser ajustados manualmente
                dot.onclick = () => window.HabitManager.toggleManualCalendarDay(habit.id, cell.dateStr);
                dot.classList.add('clickable');
            } else if (cell.status === 'future') {
                dot.textContent = cell.day;
            }
    ...
    _getFrequencyLabel: function(habit) {
        if (habit.frequencyType === 'pattern' && habit.pattern) {
            return `padrão: ${habit.pattern}`;
        }
        if (habit.frequencyType === 'weekly') {
            if (!habit.frequency || habit.frequency.length === 0 || habit.frequency.length === 7) {
                return 'todo dia';
            }
