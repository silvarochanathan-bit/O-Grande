 ...
 * VERSÃO FINAL: GOLDEN RATIO & AUDITOR FULL (Sem Abreviações).
 */
...
window.HabitView = {
    ...
    updateAuditWidget: function(habitName, finalXP, data) {
        try {
            ...
            // Formata Rótulo de Importância
            let impLabel = "Manutenção";
            if (parseFloat(k) === 1.0) impLabel = "Crítico";
            else if (parseFloat(k) === 0.5) impLabel = "Desenv.";
    ...
    createHabitElement: function(habit) {
        ...
        el.innerHTML = `
            ...
            <div class="habit-controls habit-actions">
                <button class="icon-btn" onclick="window.HabitManager.openEditModal('${habit.id}')">✏️</button>
    ...
    openEditModal: function(habitId = null) {
        ...
        impDiv.innerHTML = `
            ...
            <div class="importance-selector">
                <label><input type="radio" name="importance" value="critical" ${currentImportance === 'critical' ? 'checked' : ''}> 🔴 Crítico</label>
                <label><input type="radio" name="importance" value="development" ${currentImportance === 'development' ? 'checked' : ''}> 🟡 Desenv.</label>
                <label><input type="radio" name="importance" value="maintenance" ${currentImportance === 'maintenance' ? 'checked' : ''}> 🔵 Manut.</label>
            </div>
        `;
