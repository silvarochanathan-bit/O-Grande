 ...
 * VERSÃO: SUPER APP (HUB + 3 MÓDULOS) - Updated Export & Auto-Reset
 */
...
window.GlobalApp = {
    ...
    loadData: function() {
        try {
            ...
            if (raw) {
                ...
                this.data.meta.lastActiveDate = this.formatDate(new Date());
    ...
    processDailyRollover: function() {
        ...
        // Verifica se o HabitModel tem um reset mais robusto
        if (window.HabitModel && window.HabitModel.dailyResetCheck) {
            const wasReset = window.HabitModel.dailyResetCheck();
            if (wasReset) {
                 console.log("Global: HabitModel forçou o reset diário.");
                 // Se houve reset, força atualização visual
                 if (window.HabitManager) window.HabitManager.render();
                 if (window.GymController) window.GymController.render(); 
            }
        }
        ...
        if (this.data.lastLogin !== todayStr) {
            ...
            this.data.lastLogin = todayStr;
            // Atualiza também o meta.lastActiveDate para manter sincronia com HabitModel
            if (this.data.meta) this.data.meta.lastActiveDate = todayStr;
