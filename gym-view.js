 ...
 * Alterações: Removido o modal de Juramento e o Widget Auditor (Nota Fiscal de
 * XP). O histórico de atividade agora mostra apenas o registro (nome, detalhe,
 * hora), sem valores de XP.
...
window.GymView = {
    ...
    init: function(containerId) {
        ...
        this._injectSetTypeModal(); // Garante que o modal de tipo de série exista
        console.log("[GymView] Interface V9.0 (Lean Edition) inicializada.");
    },

    // Garante que o modal de tipo de série existe no DOM
    ...
    renderExerciseSelectionList: function(list, onSelect) {
        ...
        list.forEach(ex => {
            ...
            item.innerHTML = `
                <div><h4>${ex.name}</h4><small>${ex.category && ex.category !== 'Sem Categoria' ? ex.category + ' · ' : ''}${ex.type}</small></div>
    ...
    _renderEvoItem: function(exercise, progress) {
        ...
        // "Repetições com Peso" agora mede volume (carga × reps somado nas séries
        // válidas), não uma carga isolada — o rótulo deixa isso explícito.
        let unit;
        if (exercise.type === 'rep_weight') unit = 'kg vol.';
        else if (exercise.type === 'dur_weight' || exercise.type === 'dur_no_weight') unit = 's';
        else unit = 'kg';
