...
window.HabitManager = {
    ...
    isSystemReady: false,
    calendarMonthOffsets: {}, // { habitId: offsetFromCurrentMonth } — navegação de mês por calendário
    ...
    setupUIBinds: function() {
        ...
        // Aba "Calendários" — a renderização é pesada (monta N mini-calendários),
        // então só roda quando a aba é de fato aberta, não a cada mudança de dados.
        const navBtnCalendars = document.querySelector('.nav-btn[data-target="section-calendars"]');
        if (navBtnCalendars) {
            navBtnCalendars.addEventListener('click', () => this.renderCalendarsTab());
        }

        document.querySelectorAll('.calendar-style-picker button').forEach(btn => {
            btn.addEventListener('click', () => this.setCalendarStyle(btn.dataset.style));
        });
    },
    ...
    ensureIntegrity: function() {
        ...
        if (!window.GlobalApp.data.habitSettings) window.GlobalApp.data.habitSettings = {};
        if (!window.GlobalApp.data.habitSettings.calendarStyle) {
            window.GlobalApp.data.habitSettings.calendarStyle = 'squares';
        }
    },
    ...
    updateCounter: function(id, delta) {
        ...
        if (delta > 0 && newVal > (habit.currentOfDay || 0)) {
            ...
            if (habit.type === 'infinite' && habit.currentOfDay === 0) {
                 ...
                 habit.lastDone = window.GlobalApp.getGameDate();
            }
        }
    ...
    _completeHabit: function(habit) {
        ...
        // Só soma streak (e lembra que somou) se ainda não tinha marcado hoje
        ...
        // Guarda se essa conclusão específica somou streak, para desfazer corretamente
        habit._lastCompletionAddedStreak = streakIncremented;
    ...
    _undoCompletion: function(habit) {
        ...
        // Só desconta a sequência se a conclusão de hoje foi o que a incrementou
        if (habit._lastCompletionAddedStreak && habit.streak > 0) {
            habit.streak--;
            habit.lastDone = null;
        }
        habit._lastCompletionAddedStreak = false;
    ...
    // =========================================
    // CALENDÁRIOS DE HÁBITOS (ABA "CALENDÁRIOS")
    // =========================================

    renderCalendarsTab: function() {
        if (!this.isSystemReady) return;
        const style = window.GlobalApp.data.habitSettings?.calendarStyle || 'squares';

        document.querySelectorAll('.calendar-style-picker button').forEach(btn => {
            btn.classList.toggle('active', btn.dataset.style === style);
        });

        window.HabitView.renderCalendarsGrid(
            window.GlobalApp.data.habits || [],
            this.calendarMonthOffsets,
            style
        );
    },

    setCalendarStyle: function(style) {
        if (window.SoundManager) window.SoundManager.play('click');
        window.GlobalApp.data.habitSettings.calendarStyle = style;
        window.GlobalApp.saveData();
        this.renderCalendarsTab();
    },

    changeCalendarMonth: function(habitId, direction) {
        const current = this.calendarMonthOffsets[habitId] || 0;
        this.calendarMonthOffsets[habitId] = current + direction;
        this.renderCalendarsTab();
    },

    toggleManualCalendarDay: async function(habitId, dateStr) {
        const habit = window.GlobalApp.data.habits.find(h => h.id === habitId);
        if (!habit) return;

        if (!(await confirm(`Marcar/desmarcar manualmente o dia ${dateStr.split('-').reverse().join('/')}?`))) return;

        if (window.SoundManager) window.SoundManager.play('click');
        const changed = window.HabitModel.toggleManualLogDay(habit, dateStr);
        if (changed) this.renderCalendarsTab();
    },
