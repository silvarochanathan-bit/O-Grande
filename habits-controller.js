 ...
 * VERSÃO: V20 - ADAPTAÇÃO NEURAL OPCIONAL
 * Golden Ratio | Neural Adaptation (Manual) | Auditoria Detalhada | Time-Aware Logic.
 */
...
window.HabitManager = {
    ...
    processNeuralAdaptation: function() {
        ...
        window.GlobalApp.data.habits.forEach(h => {
            // Só processa a regressão se a adaptação estiver ativa para este hábito
    ...
    calculateMasterXP: function(habit, context = {}) {
        ...
        else if (habit.importance === 'development') K = 0.5;
        ...
        // --- ALTERAÇÃO: ADAPTAÇÃO NEURAL CONDICIONAL ---
        let M_ada = 1.0;
        ...
        if (window.HabitView && window.HabitView.updateAuditWidget) {
            window.HabitView.updateAuditWidget(habit.name, Math.round(FinalXP), auditData);
        }

        return {
            xp: Math.max(1, Math.round(FinalXP)),
    ...
    pauseTimer: function() {
        ...
        if (this.timerState.isPaused) {
            ...
            this.timerState.accumulatedBefore += Math.floor((Date.now() - this.timerState.startTime)/1000);
    ...
    handleSaveHabit: async function(e) {
        ...
        // --- NOVO: CAPTURA DO ESTADO DE ADAPTAÇÃO ---
        ...
        const habitData = {
            ...
            focus, focusStages, adaptationActive // Novo campo salvo
        };
