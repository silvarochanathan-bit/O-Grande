 ...
 * VERSÃO: V27 - CORREÇÃO DE LEITURA DE TEMPO EM CONTADORES
 * Alterações: A função updateCounter agora consome e envia o 'accumulatedTime' para o cálculo.
 */
...
window.HabitManager = {
    ...
    calculateMasterXP: function(habit, context = {}) {
        ...
        // 1. Prioriza INPUT DE TEMPO REAL (Timer ou Manual)
        ...
        // 2. Lógica de Cálculo de Volume (V)
        if (minutes > 0) {
            V = minutes / 60; 
    ...
    // --- CORREÇÃO V27: Update Counter agora consome tempo acumulado ---
    updateCounter: function(id, delta) {
        ...
        if (delta > 0 && newVal > (habit.currentOfDay || 0)) {
            // Cria o contexto base
            let xpContext = { currentStep: newVal };
            
            // VERIFICAÇÃO DE TEMPO: Se houver tempo acumulado, usa e zera.
            if (habit.accumulatedTime && habit.accumulatedTime > 0) {
                xpContext.durationSeconds = habit.accumulatedTime;
                habit.accumulatedTime = 0; // Consome o tempo para não duplicar
            }
    ...
    toggleStopwatch: function(habitId) {
        ...
        // Renderiza apenas se estiver na aba correta (segurança)
        const currentApp = window.GlobalApp.data.navigation.currentApp;
        if (currentApp === 'productivity') {
