...
window.GymModel = {
     ...
     * finalizada o timestamp real e a melhor carga válida (maior val1 entre séries
     * não-aquecimento e marcadas como feitas). Usado como base para a comparação
     * início/atual em uma janela de dias (tela de Evolução por Grupo Muscular).
     */
    _getExerciseTimeSeries: function(exerciseId) {
        ...
        history.forEach(session => {
            ...
            if (bestVal > 0) {
                series.push({ timestamp: session.startTime, value: bestVal });
            }
        });
