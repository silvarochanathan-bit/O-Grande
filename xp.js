 ...
 * VERSÃO: V12 - LEVEL UP RÁPIDO (500ms)
 */
...
window.XPManager = {
    ...
    init: function() {
        console.log("Site C: XPManager V12 Carregado");
    ...
    gainXP: async function(baseAmount, source, options = {}) {
        ...
        if (baseAmount > 0 && !options.forceFlat && !options.isMilestone) {
            ...
            if (options.streak && options.streak > 1) {
                ...
                bonusVal = Math.round(baseAmount * streakMultiplier);
        ...
        const finalXP = Math.round(currentTotal);
        const xpState = window.GlobalApp.data.xp;

        // Atualiza dados globais
        xpState.current += finalXP;
        xpState.total += finalXP;
        ...
        rawHistory.forEach((entry, realIndex) => {
            ...
            if (map.has(uniqueKey)) {
                ...
                if (!newGroup.baseXP && newGroup.count > 0) {
                    newGroup.baseXP = Math.round(newGroup.amount / newGroup.count);
                }
    ...
    checkLevelDown: async function() {
        ...
        while (xpState.current < 0) {
            ...
            xpState.current = prevMax + xpState.current;
            window.GlobalApp.saveData();
    ...
    deleteLog: async function(visualIndex) {
        ...
        if (confirmed) {
            ...
            xpState.current -= logToDelete.amount;
            xpState.total -= logToDelete.amount;
