...
window.DietManager = {
    ...
    editingPackId: null, // id do pack sendo editado no modal de Packs (null = criando novo)
    packFormItems: [], // itens do pack em edição no formulário: [{foodId, grams}]
    packFormPickerExpanded: {}, // árvore de seleção de alimento dentro do formulário de pack
    pendingPackId: null, // pack escolhido no log, aguardando ajuste de gramas antes de confirmar
    pendingPackItems: [], // cópia editável dos itens do pack pendente: [{foodId, grams}]
    ...
    ensureData: function() {
        ...
        // Packs de refeição pré-estabelecidos: { id, name, items: [{foodId, grams}] }
        if (!d.diet.mealPacks || !Array.isArray(d.diet.mealPacks)) {
            d.diet.mealPacks = [];
        }
    ...
    bindEvents: function() {
        ...
        if (btnMealPacks) btnMealPacks.onclick = () => this.openPacksModal();

        const btnClosePacks = document.getElementById('btn-close-meal-packs');
        if (btnClosePacks) btnClosePacks.onclick = () => {
            document.getElementById('modal-diet-packs').classList.add('hidden');
        };

        const btnNewPack = document.getElementById('btn-new-pack');
        if (btnNewPack) btnNewPack.onclick = () => this.startNewPack();
        ...
        if (btnCancelPackEdit) btnCancelPackEdit.onclick = () => this.cancelPackEdit();

        const btnAddItemToPack = document.getElementById('btn-open-pack-item-picker');
        if (btnAddItemToPack) btnAddItemToPack.onclick = () => this.openPackItemPicker();
        ...
        // --- Usar Pack no Registro de Refeição ---
        const btnOpenPackPicker = document.getElementById('btn-open-log-pack-picker');
        if (btnOpenPackPicker) btnOpenPackPicker.onclick = () => this.openLogPackPicker();
        ...
        if (btnUseDefaultPack) btnUseDefaultPack.onclick = () => this.confirmAddPack(false);

        const btnUseAdjustedPack = document.getElementById('btn-use-adjusted-pack');
        if (btnUseAdjustedPack) btnUseAdjustedPack.onclick = () => this.confirmAddPack(true);
    ...
    // =========================================================================
    // PACKS DE REFEIÇÃO (COMBOS PRÉ-ESTABELECIDOS)
    // =========================================================================

    // Busca um alimento do foodDb pelo seu id estável (não pelo índice, que pode mudar)
    _findFoodById: function(foodId) {
        return window.GlobalApp.data.diet.foodDb.find(f => f.id === foodId) || null;
    },

    openPacksModal: function() {
        this.cancelPackEdit();
    ...
    renderPacksList: function() {
        ...
        const packs = window.GlobalApp.data.diet.mealPacks;
        container.innerHTML = '';

        if (packs.length === 0) {
            container.innerHTML = '<div style="padding:20px; text-align:center; color:var(--diet-text-sub); font-size:0.85rem;">Nenhum pack criado ainda.</div>';
            return;
        }
        ...
        packs.forEach(pack => {
            const itemsSummary = pack.items
                .map(it => {
                    const f = this._findFoodById(it.foodId);
                    return f ? `${f.name} (${it.grams}g)` : '(alimento removido)';
                })
                .join(', ');
            ...
            div.innerHTML = `
                <div>
                    ...
                    <div class="db-food-macros" style="opacity:0.7; font-family:inherit; text-transform:none; letter-spacing:0;">
                        <span>${itemsSummary || 'Sem itens'}</span>
                    </div>
                </div>
                <div class="db-item-actions">
                    <button class="btn-edit-db-item" onclick="event.stopPropagation(); window.DietManager.editPack('${pack.id}')">✏️</button>
                    <button class="btn-delete-db-item" onclick="event.stopPropagation(); window.DietManager.deletePack('${pack.id}')">🗑️</button>
                </div>
            `;
            container.appendChild(div);
        });
    ...
    startNewPack: function() {
        this.editingPackId = null;
        this.packFormItems = [];
        document.getElementById('new-pack-name').value = '';
        this.renderPackFormItems();
        const btn = document.querySelector('#form-diet-pack button[type="submit"]');
        if (btn) btn.textContent = "+ Salvar Pack";
        document.getElementById('diet-pack-form-section').classList.remove('hidden');
    },

    editPack: function(packId) {
        const pack = window.GlobalApp.data.diet.mealPacks.find(p => p.id === packId);
        if (!pack) return;

        this.editingPackId = packId;
        this.packFormItems = pack.items.map(it => ({ ...it })); // cópia
    ...
    deletePack: async function(packId) {
        if (await confirm('Apagar este pack? Os alimentos que o compõem não serão afetados.')) {
            const d = window.GlobalApp.data.diet;
            d.mealPacks = d.mealPacks.filter(p => p.id !== packId);
            window.GlobalApp.saveData();
            if (this.editingPackId === packId) this.cancelPackEdit();
            this.renderPacksList();
        }
    },

    // Abre a árvore de alimentos (fullscreen) para escolher qual item entra no pack
    ...
    renderPackItemPickerTree: function() {
        ...
        this._renderCategoryNode(root, container, 0, this.packFormPickerExpanded,
            (path) => { this.packFormPickerExpanded[path] = !this.packFormPickerExpanded[path]; this.renderPackItemPickerTree(); },
            {
                renderItem: (f, idx, depth) => {
                    const div = document.createElement('div');
                    ...
                    `;
                    div.onclick = () => this.addItemToPackForm(f.id);
                    return div;
                }
            },
            false
    ...
    addItemToPackForm: function(foodId) {
        this.packFormItems.push({ foodId, grams: 100 });
        this.renderPackFormItems();
        document.getElementById('modal-diet-pack-item-picker').classList.add('hidden');
    },

    removeItemFromPackForm: function(itemIndex) {
        this.packFormItems.splice(itemIndex, 1);
        this.renderPackFormItems();
    },

    updatePackFormItemGrams: function(itemIndex, grams) {
        const g = parseFloat(grams);
        this.packFormItems[itemIndex].grams = isNaN(g) ? 0 : g;
    },

    renderPackFormItems: function() {
        const container = document.getElementById('pack-form-items-list');
        if (!container) return;
        container.innerHTML = '';

        if (this.packFormItems.length === 0) {
            container.innerHTML = '<div style="padding:12px; text-align:center; color:var(--diet-text-sub); font-size:0.8rem;">Nenhum alimento adicionado ao pack ainda.</div>';
            return;
        }

        this.packFormItems.forEach((item, idx) => {
            const f = this._findFoodById(item.foodId);
            const div = document.createElement('div');
            div.className = 'diet-pack-form-item';
            div.innerHTML = `
                <span class="diet-pack-form-item-name">${f ? f.name : '(alimento removido)'}</span>
                <input type="number" class="gym-form-input diet-pack-form-item-grams" value="${item.grams}" step="0.1" min="0"
                       onchange="window.DietManager.updatePackFormItemGrams(${idx}, this.value)">
                <span class="diet-pack-form-item-unit">g</span>
                <button type="button" class="btn-delete-db-item" onclick="window.DietManager.removeItemFromPackForm(${idx})">🗑️</button>
            `;
            container.appendChild(div);
        });
    },
    ...
    savePack: function() {
        ...
        if (this.packFormItems.length === 0) { alert('Adicione pelo menos um alimento ao pack!'); return; }

        const d = window.GlobalApp.data.diet;

        if (this.editingPackId !== null) {
            const pack = d.mealPacks.find(p => p.id === this.editingPackId);
            if (pack) {
                pack.name = name;
                pack.items = this.packFormItems.map(it => ({ ...it }));
            }
        } else {
            d.mealPacks.push({
                id: 'pack_' + Date.now(),
                name,
                items: this.packFormItems.map(it => ({ ...it }))
            });
        }
    ...
    // --- Uso de Pack no Registro de Refeição ---
    ...
    renderLogPackPickerList: function() {
        ...
        if (packs.length === 0) {
            container.innerHTML = '<div style="padding:20px; text-align:center; color:var(--diet-text-sub); font-size:0.85rem;">Nenhum pack criado. Crie um em "Meus Packs".</div>';
            return;
        }
        ...
        packs.forEach(pack => {
            const itemsSummary = pack.items
                .map(it => {
                    const f = this._findFoodById(it.foodId);
                    return f ? `${f.name} (${it.grams}g)` : null;
                })
                .filter(Boolean)
                .join(', ');
    ...
    // Alimento escolhido: abre a tela de ajuste de gramas antes de confirmar a adição
    selectPackForLog: function(packId) {
        const pack = window.GlobalApp.data.diet.mealPacks.find(p => p.id === packId);
        if (!pack) return;

        this.pendingPackId = packId;
        this.pendingPackItems = pack.items.map(it => ({ ...it })); // cópia editável

        document.getElementById('modal-diet-log-pack-picker').classList.add('hidden');
        this.renderPackAdjustScreen(pack.name);
        document.getElementById('modal-diet-pack-adjust').classList.remove('hidden');
    },

    renderPackAdjustScreen: function(packName) {
        document.getElementById('pack-adjust-title').textContent = `Ajustar "${packName}"`;
        const container = document.getElementById('pack-adjust-items-list');
        container.innerHTML = '';

        this.pendingPackItems.forEach((item, idx) => {
            const f = this._findFoodById(item.foodId);
            const div = document.createElement('div');
            div.className = 'diet-pack-form-item';
            div.innerHTML = `
                <span class="diet-pack-form-item-name">${f ? f.name : '(alimento removido)'}</span>
                <input type="number" class="gym-form-input diet-pack-form-item-grams" value="${item.grams}" step="0.1" min="0"
                       onchange="window.DietManager.updatePendingPackItemGrams(${idx}, this.value)">
                <span class="diet-pack-form-item-unit">g</span>
            `;
            container.appendChild(div);
        });
    ...
    updatePendingPackItemGrams: function(itemIndex, grams) {
        const g = parseFloat(grams);
        this.pendingPackItems[itemIndex].grams = isNaN(g) ? 0 : g;
    },

    // useAdjusted=false usa as gramas originais do pack; true usa os valores editados na tela
    confirmAddPack: function(useAdjusted) {
        const pack = window.GlobalApp.data.diet.mealPacks.find(p => p.id === this.pendingPackId);
        if (!pack) return;

        const itemsToAdd = useAdjusted ? this.pendingPackItems : pack.items;
        ...
        itemsToAdd.forEach(item => {
            const baseFood = this._findFoodById(item.foodId);
            if (!baseFood || item.grams <= 0) return;
